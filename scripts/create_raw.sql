...
-- 6. Minimum grants
-----------------------------------------------------------------
GRANT USAGE ON WAREHOUSE IDENTIFIER($WH_NAME) TO ROLE IDENTIFIER($DBT_ROLE);
GRANT OPERATE ON WAREHOUSE IDENTIFIER($WH_NAME) TO ROLE IDENTIFIER($DBT_ROLE);

GRANT USAGE ON DATABASE IDENTIFIER($DB_NAME) TO ROLE IDENTIFIER($DBT_ROLE);
GRANT USAGE ON SCHEMA IDENTIFIER($DB_NAME||'.'||$RAW_SCHEMA) TO ROLE IDENTIFIER($DBT_ROLE);

GRANT SELECT ON ALL TABLES IN SCHEMA IDENTIFIER($DB_NAME||'.'||$RAW_SCHEMA) 
  TO ROLE IDENTIFIER($DBT_ROLE);
GRANT SELECT ON FUTURE TABLES IN SCHEMA IDENTIFIER($DB_NAME||'.'||$RAW_SCHEMA) 
  TO ROLE IDENTIFIER($DBT_ROLE);
